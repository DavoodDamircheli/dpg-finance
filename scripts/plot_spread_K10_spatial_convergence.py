#!/usr/bin/env python3
"""
MB-2b Step 2: LaTeX table + log-log convergence figure for spread call K=10.

Reads: results_v5_benchmarks/csv/spread_K10_spatial_convergence.csv
Writes:
...
Usage:
  python3 scripts/plot_spread_K10_spatial_convergence.py
"""
...
CSV_PATH = "results_v5_benchmarks/csv/spread_K10_spatial_convergence.csv"
TEX_PATH = "results_v5_benchmarks/tex/table_spread_K10_spatial.tex"
FIG_PATH = "results_v5_benchmarks/figures/fig_spread_K10_spatial_convergence.pdf"
...
def write_latex(rows, atm_quad):
    ...
    atm_ref_str = f"{atm_quad:.4f}" if atm_quad is not None else "---"

    lines = []
    lines.append(r"\newcommand{\tableSpreadK10SpatialConvergence}{%")
    lines.append(r"\begin{table}[ht]")
    lines.append(r"\centering")
    lines.append(r"\caption{")
    lines.append(r"  Spatial convergence of the DPG spread call price with nonzero strike.")
    lines.append(r"  Ultraweak formulation (no 2D primal solver available).")
    lines.append(r"  Payoff $(S_1 - S_2 - K_s)^+$ with $K_s=10$;")
    lines.append(r"  log-price coordinates $x_i = \log(S_i/100)$,")
    lines.append(r"  domain $[-3,3]^2$,")
    lines.append(r"  $\sigma_1=\sigma_2=0.20$, $\rho=0.50$, $r=0.05$, $T=1$.")
    lines.append(r"  Trial: $L^2$ degree $p=0$; enrichment $\Delta p=2$ (test order 3).")
    lines.append(
        rf"  Benchmark: 2D Gauss-Hermite quadrature ($n_{{\rm quad}}={N_QUAD}$)"
        r" of the bivariate lognormal terminal distribution, $U^*=" + atm_ref_str + r"$."
    )
    lines.append(r"  $N_t = 2N$.}")
    lines.append(r"\label{tab:spread_K10_spatial}")
    ...
    for r in rows:
        ...
        lines.append(
            rf"  {N} & {Nt} & ${l2_str}$ & {eoc_str} & ${atm_str}$ & ${aq_str}$ \\"
        )
...
def write_figure(rows):
    ...
    if valid:
        hv, ev = zip(*valid)
        ax.loglog(hv, ev, "s-", color="tab:orange", linewidth=1.8, markersize=6,
                  label="Ultraweak DPG")
    ...
    ax.set_title(
        rf"Spread call $(S_1-S_2-{int(K_SPREAD)})^+$: $L^2$ convergence, $\rho=0.5$",
        fontsize=10
    )
...
def main():
    if not os.path.exists(CSV_PATH):
        print(f"ERROR: {CSV_PATH} not found. Run run_spread_K10_spatial_convergence.py first.",
              file=sys.stderr)
        sys.exit(1)
