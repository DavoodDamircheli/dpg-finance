#!/usr/bin/env python3
"""
MB-2a: Spatial convergence — basket average call, ultraweak 2D DPG.

Payoff: (0.5*S1 + 0.5*S2 - K)^+, K=100
...
Outputs:
  ...
  (Run plot_basket_spatial_convergence.py next for LaTeX + figure)

Usage (inside Singularity container):
  python3 scripts/run_basket_spatial_convergence.py [--np 8]
"""
...
# ── Import quadrature reference from MB-0 ────────────────────────────────────
...
N_QUAD = 64   # GH quadrature order for Python reference (consistent with MB-0)

OUTPUT_CSV = "results_v5_benchmarks/csv/basket_spatial_convergence.csv"
LOG_DIR    = "results_v5_benchmarks/logs"


def atm_quad_reference():
    """Bivariate lognormal quadrature price at S1=S2=K (ATM)."""
    return bivlognormal_quadrature_price(
        S_REF, S_REF, K, T, R, SIG1, SIG2, RHO,
        basket_payoff, n_quad=N_QUAD
    )
...
def verify(rows, atm_exact):
    ...
    if len(l2s) >= 2:
        ...
        tag = "PASS" if mono else f"WARNING: L2 NOT monotone FAILED"
...
def main():
    ...
    print(f"ATM quadrature reference (n_quad={N_QUAD}): {atm_quad:.8f}")

    mesh_seq = [128, 192, 256, 384, 512]
    print(f"\nMB-2a Spatial convergence: basket average call (0.5*S1+0.5*S2-K)^+")
    print(f"  Domain: [-{DOMAIN_HALF},{DOMAIN_HALF}]^2  rho={RHO}  sigma={SIG1}  r={R}  T={T}  K={K}")
    print(f"  ATM_quad (bivariate lognormal, n_quad={N_QUAD}): {atm_quad:.6f}")
    ...
    with open(OUTPUT_CSV, "w") as f:
        f.write("# Basket average call (0.5*S1+0.5*S2-K)^+ spatial convergence\n")
        f.write(f"# sigma1=sigma2={SIG1} rho={RHO} r={R} T={T} K={K}  ATM_quad={atm_quad:.8f}\n")
        f.write(header + "\n")
        for r in rows:
    ...
    print("Done. Run plot_basket_spatial_convergence.py next.")
