#!/usr/bin/env python3
"""
MBP-4a Step 1: Temporal convergence — basket-average call,
...
ATM ref: Python GH n_quad=64 (9.45796645)

Spatial-floor check (per spec):
  floor = L2(N=384, Nt=1024) from this run.
  From MBP-3a: L2(N=384, Nt=768) = 2.038e+01.
...
SPATIAL_FLOOR_MBP3 = 20.376       # MBP-3a: N=384, Nt=768
DOMAIN_HALF        = 3.0
FORMULATION        = "primal"

OUTPUT_CSV = "results_v5_benchmarks_primal/csv/basket_temporal_convergence_primal.csv"
LOG_PATH   = "results_v5_benchmarks_primal/logs/MBP4a_basket_temporal_run.log"
...
def spatial_floor_check(fixed_N, l2_nt32, l2_nt1024):
    log("─" * 60)
    log("Spatial-floor check (per spec MBP-4a):")
    log(f"  spatial floor (N={fixed_N}, Nt=1024) = {l2_nt1024:.6e}")
    log(f"  MBP-3a reference  (N={fixed_N}, Nt=768) = {SPATIAL_FLOOR_MBP3:.6e}")
    if l2_nt32 is None:
    ...
    if ratio < 0.10:
        ...
        log(f"  Kink-dominated pattern (see MBP-2/MB-2): floor ~ L2(Nt=32).")
        log(f"  Bumping to N=512 gives same floor (~2.035e+01 from MBP-3a). No benefit.")
...
def main():
    ...
    log("=" * 65)
    log("MBP-4a: Basket-average call temporal convergence — PRIMAL Galerkin")
    log("=" * 65)
    log(f"Payoff      : (0.5*S1 + 0.5*S2 - K)^+, K=100")
    log(f"Fixed mesh  : N={fixed_N}  domain=[-3,3]^2")
    log(f"Nt sequence : {nt_seq}")
    log(f"ATM_quad    : {ATM_QUAD}  (Python GH n=64)")
    log(f"Spatial floor (MBP-3a, N={fixed_N}, Nt=768): {SPATIAL_FLOOR_MBP3:.6e}")
    ...
    with open(OUTPUT_CSV, "w") as f:
        f.write("# Basket-average call temporal convergence — primal H1(1) Galerkin\n")
        f.write(f"# N={fixed_N}  sig1=sig2=0.2  r=0.05  T=1  K=100  rho=0.5  ATM_quad={ATM_QUAD}\n")
        f.write(header + "\n")
        for row in rows:
    ...
    log("Done. Run plot_basket_temporal_convergence_primal.py next.")
