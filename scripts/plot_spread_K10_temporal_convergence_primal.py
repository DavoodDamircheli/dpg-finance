#!/usr/bin/env python3
"""
MBP-4b Step 2: LaTeX table + log-log figure for spread call (S1-S2-10)^+
temporal convergence, PRIMAL 2D H1(1) Galerkin.

Reads:  results_v5_benchmarks_primal/csv/spread_K10_temporal_convergence_primal.csv
Writes:
  results_v5_benchmarks_primal/tex/table_spread_K10_temporal_primal.tex
      (command: \tableSpreadK10TemporalConvergencePrimal)
...
CSV_PATH = "results_v5_benchmarks_primal/csv/spread_K10_temporal_convergence_primal.csv"
TEX_PATH = "results_v5_benchmarks_primal/tex/table_spread_K10_temporal_primal.tex"
FIG_PATH = "results_v5_benchmarks_primal/figures/fig_spread_K10_temporal_convergence_primal.pdf"
...
def write_latex(rows):
    ...
    lines.append(r"  Temporal convergence of the spread call option price,")
    lines.append(r"  primal $H^1(1)$ Galerkin formulation. Fixed spatial mesh $N=384$.")
    lines.append(r"  Payoff: $(S_1-S_2-\tilde{K})^+$, $\tilde{K}=10$.")
    lines.append(r"  Domain $[-3,3]^2$ in log-price coordinates,")
    lines.append(r"  $\sigma_1=\sigma_2=0.20$, $\rho=0.50$, $r=0.05$, $T=1$, $K=100$ ($S_{\rm ref}$).")
    ...
    if floor_val is not None:
        lines.append(
            ...
            r"  (kink at $S_1-S_2=\tilde{K}$ dominates; floor is $\sim\!100\%$ of $L^2(\Delta\tau_{\rm coarse})$).}"
        )
    else:
        lines.append(r"  (L2 dominated by kink-induced spatial floor; no temporal convergence expected.)}")
    lines.append(r"\label{tab:spread_K10_temporal_primal}")
...
def write_figure(rows):
    ...
    ax.loglog(dv, ev, "D-", color="tab:red", linewidth=1.8, markersize=6,
    ...
    ax.set_title(
        r"Spread call ($\tilde{K}=10$): primal $H^1(1)$, $N=384$, $\rho=0.5$",
        fontsize=10
    )
